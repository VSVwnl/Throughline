...
export default function ResultView() {
  ...
  const active: ArchetypeMatch =
    result.paraFirstOrder.find((m) => m.archetype.id === activeId) ??
    ...
    <main className="flex-1 px-6 py-12 max-w-6xl mx-auto w-full">
      ...
      <section>
        ...
        <h1 className="text-3xl sm:text-4xl font-semibold tracking-tight">
          ...
          <span className="text-paralympic">{result.paraFirstOrder[0].archetype.name}</span>
          {" "}and{" "}
          <span className="text-olympic">{result.paraFirstOrder[1].archetype.name}</span>.
        </h1>
      ...
      <section className="mt-16">
        ...
        />
        <StoryStudio archetype={active.archetype} paraLeading={active.archetype.paraLeaning} />
      </section>
      ...
      {chatOpen && (
        <ChatPanel archetype={active.archetype} onClose={() => setChatOpen(false)} />
      )}
    </main>
  );
}
      ...
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          <div className={`font-mono text-xs uppercase tracking-wider mb-1 text-${accent}`}>
            {isPrimary ? "First card · Paralympic-leaning" : "Second card"}
          </div>
